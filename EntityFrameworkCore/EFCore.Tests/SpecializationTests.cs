...
namespace EFCore.Tests
{
    public class SpecializationTests
    {
        ...
        [Fact]
        public void Specialization_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 8;

            // Act [calculate actual values]
            int actual = context.Specialization.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.Specialization) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(ISpecialization.Description))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new Specialization();
        ...
        [Theory]
        [InlineData(nameof(ISpecialization.Description), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.Specialization.ToList();
