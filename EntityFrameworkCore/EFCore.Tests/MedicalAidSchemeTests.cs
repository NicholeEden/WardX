...
namespace EFCore.Tests
{
    public class MedicalAidSchemeTests
    {
        ...
        [Fact]
        public void MedicalAidScheme_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 5;

            // Act [calculate actual values]
            int actual = context.MedicalAidScheme.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.MedicalAidScheme) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(IMedicalAidScheme.Name))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new MedicalAidScheme();
        ...
        [Theory]
        [InlineData(nameof(IMedicalAidScheme.Name), 128)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.MedicalAidScheme.ToList();
