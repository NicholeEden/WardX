...
{
    public class MedicalAidPlanTests
    {
        ...
        public MedicalAidPlanTests(ITestOutputHelper output)
        {
            this.output = output;
        ...
        [Fact]
        public void MedicalAidPlan_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 10;

            // Act [calculate actual values]
            int actual = context.MedicalAidPlan.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.MedicalAidPlan) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(IMedicalAidPlan.Name))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new MedicalAidPlan();
        ...
        [Theory]
        [InlineData(nameof(IMedicalAidPlan.Name), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.MedicalAidPlan.ToList();
