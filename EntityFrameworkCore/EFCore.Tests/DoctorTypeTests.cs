...
namespace EFCore.Tests
{
    public class DoctorTypeTests
    {
        ...
        public DoctorTypeTests(ITestOutputHelper output)
        {
            this.output = output;
        ...
        [Fact]
        public void DoctorType_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 11;

            // Act [calculate actual values]
            int actual = context.DoctorType.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.DoctorType) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(IDoctorType.Description))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new DoctorType();
        ...
        [Theory]
        [InlineData(nameof(IDoctorType.Description), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.DoctorType.ToList();
