...
namespace EFCore.Tests
{
    public class MonthTests
    {
        ...
        public MonthTests(ITestOutputHelper output)
        {
            this.output = output;
        ...
        [Fact]
        public void Month_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 12;

            // Act [calculate actual values]
            int actual = context.Month.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.Month) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(IMonth.MonthName))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new Month();
        ...
        [Theory]
        [InlineData(nameof(IMonth.MonthName), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.Month.ToList();
