...
namespace EFCore.Tests
{
    public class NurseTypeTests
    {
        ...
        public NurseTypeTests(ITestOutputHelper output)
        {
            this.output = output;
        ...
        [Fact]
        public void NurseType_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 5;

            // Act [calculate actual values]
            int actual = context.NurseType.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.NurseType) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(INurseType.Description))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new NurseType();
        ...
        [Theory]
        [InlineData(nameof(INurseType.Description), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.NurseType.ToList();
