...
namespace EFCore.Tests
{
    public class MedicalConditionTests
    {
        ...
        [Fact]
        public void MedicalCondition_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 15;

            // Act [calculate actual values]
            int actual = context.MedicalCondition.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.MedicalCondition) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(IMedicalCondition.Name))]
        [InlineData(nameof(IMedicalCondition.Symptom))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new MedicalCondition();
        ...
        [Theory]
        [InlineData(nameof(IMedicalCondition.Name), 1024)]
        [InlineData(nameof(IMedicalCondition.Symptom), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.MedicalCondition.ToList();
