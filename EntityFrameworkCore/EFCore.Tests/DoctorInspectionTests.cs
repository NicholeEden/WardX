...
{
    public class DoctorInspectionTests
    {// provides user defined test output
        private readonly ITestOutputHelper output;

        public DoctorInspectionTests(ITestOutputHelper output)
        {
            this.output = output;
        }

        [Theory]
        [InlineData(nameof(IDoctorInspection.Comments))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new DoctorInspection();
        ...
        [Theory]
        [InlineData(nameof(IDoctorInspection.Comments), 1024)]
        ...
        [Theory]
        [InlineData(nameof(DoctorInspection.Doctor))]
        [InlineData(nameof(DoctorInspection.Patient))]
