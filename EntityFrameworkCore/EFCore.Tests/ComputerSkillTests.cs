...
{
    public class ComputerSkillTests
    {
        ...
        public ComputerSkillTests(ITestOutputHelper output)
        {
            this.output = output;
        ...
        [Fact]
        public void ComputerSkill_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 4;

            // Act [calculate actual values]
            int actual = context.ComputerSkill.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.ComputerSkill) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(IComputerSkill.Application))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new ComputerSkill();
        ...
        [Theory]
        [InlineData(nameof(IComputerSkill.Application), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.ComputerSkill.ToList();
