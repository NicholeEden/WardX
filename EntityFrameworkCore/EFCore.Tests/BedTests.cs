...
namespace EFCore.Tests
{
    public class BedTests
    {
        ...
        public BedTests(ITestOutputHelper output)
        {
            this.output = output;
        ...
        [Fact]
        public void Bed_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 10;

            // Act [calculate actual values]
            int actual = context.Bed.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.Bed) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(IBed.Description))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new Bed();
        ...
        [Theory]
        [InlineData(nameof(IBed.Description), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.Bed.ToList();
