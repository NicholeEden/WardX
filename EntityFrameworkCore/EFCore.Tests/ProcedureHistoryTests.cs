...
{
    public class ProcedureHistoryTests
    {
        // provides user defined test output
        private readonly ITestOutputHelper output;

        public ProcedureHistoryTests(ITestOutputHelper output)
        {
            this.output = output;
        }

        [Theory]
        [InlineData(nameof(IProcedureHistory.Notes))]
        [InlineData(nameof(IProcedureHistory.ProcedureDescription))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new ProcedureHistory();
        ...
        [Theory]
        [InlineData(nameof(IProcedureHistory.Notes), 1024)]
        [InlineData(nameof(IProcedureHistory.ProcedureDescription), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = new ProcedureHistory
            {
                Notes = "These are some pretty awesome notes",
                ProcedureDescription = "Awesome description"
            };
        ...
        [Theory]
        [InlineData(nameof(ProcedureHistory.Patient))]
