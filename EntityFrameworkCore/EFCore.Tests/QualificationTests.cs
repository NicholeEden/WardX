...
namespace EFCore.Tests
{
    public class QualificationTests
    {
        ...
        public QualificationTests(ITestOutputHelper output)
        {
            this.output = output;
        ...
        [Fact]
        public void Qualification_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 4;

            // Act [calculate actual values]
            int actual = context.Qualification.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.Qualification) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(IQualification.Description))]
        [InlineData(nameof(IQualification.Title))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new Qualification();
        ...
        [Theory]
        [InlineData(nameof(IQualification.Description), 1024)]
        [InlineData(nameof(IQualification.Title), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.Qualification.ToList();
