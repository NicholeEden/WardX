...
namespace EFCore.Tests
{
    public class BarTests
    {
        ...
        [Fact]
        public void Bar_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 7;

            // Act [calculate actual values]
            int actual = context.Bar.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.Bar) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(IBar.Colour))]
        [InlineData(nameof(IBar.Description))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new Bar();
        ...
        [Theory]
        [InlineData(nameof(IBar.Colour), 1024)]
        [InlineData(nameof(IBar.Description), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.Bar.ToList();
