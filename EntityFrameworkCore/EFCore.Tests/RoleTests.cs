...
namespace EFCore.Tests
{
    public class RoleTests
    {
        ...
        [Fact]
        public void Role_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 1;

            // Act [calculate actual values]
            int actual = context.Role.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.Role) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(IRole.RoleName))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new Role();
        ...
        [Theory]
        [InlineData(nameof(IRole.RoleName), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.Role.ToList();
        ...
        [Theory]
        [InlineData(nameof(Role.UserRoles))]
