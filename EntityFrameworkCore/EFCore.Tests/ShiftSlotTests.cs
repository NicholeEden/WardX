...
namespace EFCore.Tests
{
    public class ShiftSlotTests
    {
        ...
        public ShiftSlotTests(ITestOutputHelper output)
        {
            this.output = output;
        ...
        [Fact]
        public void ShiftSlot_Has_Data_Seeded()
        {
            // Arrange [declare expected values]
            int expected = 2;

            // Act [calculate actual values]
            int actual = context.ShiftSlot.Count();

            // Assert [compare actual value with expected value]
            output.WriteLine($"There should be at least { expected } row(s) of data in the { nameof(context.ShiftSlot) } table");
            Assert.True(actual >= expected);
        }

        [Theory]
        [InlineData(nameof(IShiftSlot.Description))]
        public void Data_Annotation_Required(string nullableColumn)
        {
            // Arrange - Create a null instance
            var domainModel = new ShiftSlot();
        ...
        [Theory]
        [InlineData(nameof(IShiftSlot.Description), 1024)]
        ...
        public void Data_Annotation_Valid()
        {
            ...
            var domainModel = context.ShiftSlot.ToList();
